...
public class StripeService {
    ...
    public String createCheckoutSession(Commande commande,
                                        List<CommandeProduit> items) {
        try {
            // ✅ Construire les line items avec quantités réelles
            List<SessionCreateParams.LineItem> lineItems = items.stream()
                    .map(item -> SessionCreateParams.LineItem.builder()
                            .setQuantity((long) item.getQuantite())
                            .setPriceData(SessionCreateParams.LineItem.PriceData.builder()
                                    .setCurrency(currency)
                                    .setUnitAmount(Math.round(item.getPrixUnitaire() * 100))
                                    .setProductData(
            ...
            SessionCreateParams params = SessionCreateParams.builder()
                    ...
                    // ✅ clientReferenceId pour retrouver la commande dans le webhook
    ...
    public void createRefund(String paymentIntentId) {
        try {
            ...
            throw new RuntimeException("Erreur remboursement Stripe : " + e.getMessage());
        }
    }
}
