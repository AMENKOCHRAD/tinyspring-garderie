...
public class DatasetGeneratorServiceImpl implements IDatasetGeneratorService {
    ...
    private List<Map<String, String>> genererExemplesDashboard(DashboardStatsDTO stats) {
        ...
        ex.add(creer("Donne-moi un résumé de la situation RH", ctx,
                "📊 RH TINYSPRING GARDERIE\n\n" +
    ...
    private List<Map<String, String>> genererExemplesAbsences(
        ...
        List<Map<String, String>> ex = new ArrayList<>();
        String ctx = buildContexteAbsences(absences);
        LocalDate maintenant = LocalDate.now();

        ex.add(creer("Génère le rapport mensuel des absences", ctx,
                buildRapportMensuel(absences, stats, maintenant)));
        ex.add(creer("Liste toutes les demandes en attente", ctx,
                buildDemandesEnAttente(absences)));
        ex.add(creer("Analyse les absences par type", ctx,
    ...
    private List<Map<String, String>> genererExemplesQuotas(
        ...
        ex.add(creer("Comment fonctionne le moteur de règles ?", ctx,
                "MOTEUR DE RÈGLES — FONCTIONNEMENT\n\n" +
                        "1. DÉLAI DE PRÉVENANCE\n2. QUOTA ANNUEL\n3. CHEVAUCHEMENT\n4. EFFECTIF MINIMUM\n\n" +
                        "Toutes OK → AUTO-APPROUVÉ | Règle échoue → AUTO-REFUSÉ | Admin → TRANSMIS ADMIN"));
        return ex;
    }
    ...
    private String buildUrgencesRH(DashboardStatsDTO stats) {
        StringBuilder sb = new StringBuilder("⚠️ URGENCES\n");
        boolean urgent = false;
        if (stats.getAbsencesEnAttente() > 0) {
            sb.append("• ").append(stats.getAbsencesEnAttente()).append(" demande(s) en attente\n");
            urgent = true;
        }
        if (stats.getAnimatricesInactives() > 0) {
            sb.append("• ").append(stats.getAnimatricesInactives()).append(" animatrice(s) inactive(s)\n");
            urgent = true;
        }
    ...
    private String buildContexteAbsences(List<AbsenceConge> absences) {
    ...
    private String buildAlertesQuotas(List<Animatrice> animatrices) {
        ...
        for (Animatrice a : animatrices) {
            int joursUtilises = absenceCongeRepository.findByAnimatriceId(a.getId()).stream()
                    ...
                    .mapToInt(ac -> ac.getNbJours() != null ? ac.getNbJours() : 0).sum();
            if (joursUtilises >= 25) {
                alerteTrouvee = true;
    ...
    private String buildBilanComplet(DashboardStatsDTO stats) {
        return "📊 BILAN RH COMPLET — TINYSPRING GARDERIE\n\n" +
                ...
                "RECOMMANDATIONS\n" +
                (stats.getAbsencesEnAttente() > 0 ?
                        "⚠️ Traiter " + stats.getAbsencesEnAttente() + " demande(s)\n" : "") +
